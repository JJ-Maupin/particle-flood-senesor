...
const int samples = 20; // number of samples we use to avg our data
...
void loop() {
  ...
  Particle.publish(eventName, "Test", PRIVATE); // inserted this line on May 26, 2021 - AJK
...
bool allReadingsToServer(float old, float newAnalog, float oldAvg, float newAnalogAvg, float serialValue, float temp) {
  ...
  String msg = String::format("{\"id\":" + sensorID + " , \"wl1\": %0.5f, \"wl2\": %0.5f, \"wl3\": %0.2f, \"temp\": %0.2f, \"b\": %0.2f}",
                              oldAvg, newAnalog, newAnalogAvg, temp, fuelGauge.getSoC());

  // return Particle.publish(eventName, msg, PRIVATE);
  
  return Particle.publish(eventName, msg,WITH_ACK); // Added this code on 06/17/21 - AJK
}
...
long calculateSleepCycle() {
  ...
  */
  ...
  while(!Particle.publish(eventName, "{Sleep time: " + String((int)timeToInterval) + "}", PUBLIC, WITH_ACK )){}
...
int weatherUpdater(String data){
    ...
    while(!Particle.publish(eventName, "{Sleep time: " + String((int)PERIOD) + "}", PUBLIC, WITH_ACK )){}
